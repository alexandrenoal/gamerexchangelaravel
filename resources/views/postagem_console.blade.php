                ...
                <div class="card-header"><i class="bi bi-joystick"></i> {{ __('Postagem de Consoles') }} </div>
                    ...
                    <div id="event-create-container" class="col-md-6 offset-md-3"><br>
                        ...
                        @foreach($consoles as $a)    
                            <hr>    
                            <p><a href="/vitrine/{{ $a->username }}"><b>Console:</b> {{ $a->nome }} - <b>Desenvolvedora:</b> {{ $a->plataforma }} - <b>Usuário:</b> {{ $a->username }} - <b>Data da publicação:</b> {{ $a->created_at }}</p>
                                <div id="image-container" class="col-md-6">
                                    <img src="/img/consoles/{{ $a->image }}" class="imagepostconsole">
                                </div>            
                        @endforeach
                        </div>       
                    </div>
